  ...
  const OrderDetailsPageComponent = ({ getOrder }) => {
    ...
    useEffect(() => {
        ...
        .then((order) => {
            ...
            order.isDelivered ? setIsDelivered(order.deliverdAt): setIsDelivered(false)
            setCartSubtotal(order.orderTotal.cardSubtotal)
        ...
        <Row className="mt-4">
          ...
          <Col md={4}>
            <ListGroup>
              ...
              <ListGroup.Item>
                Items price (after tax): <span className="fw-bold">{cardSubtotal}</span>
              </ListGroup.Item>
              <ListGroup.Item>
