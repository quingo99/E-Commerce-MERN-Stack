...
//--------------------VARIABLES--------------------------
  ...
  const [user, setUser] = useState({userInfoFromRedux});
    ...
    if (event.currentTarget.checkValidity() === true && password === confirmPassword) {
      ...
      .then(data => {
            ...
            if(userInfo.doNotLogOut){
                localStorage.setItem("userInfo", JSON.stringify({doNotLogOut: userInfo.doNotLogOut, ...data.userUpdated}));
            } else{
                sessionStorage.setItem("userInfo", JSON.stringify({doNotLogOut: userInfo.doNotLogOut, ...data.userUpdated}));
            }
          }
        )  
